...
export function Showcase() {
  const { gallery, winners, projects, stats, submitCta, eventDetails } = showcaseContent;
  const [selectedFilter, setSelectedFilter] = useState('All Projects');
  const [selectedWinnerYear, setSelectedWinnerYear] = useState('All Years');
  const visibleProjects = selectedFilter === 'All Projects'
    ? projects
    : projects.filter((project) => project.type === selectedFilter);
  const winnerYears = [...new Set(winners.images.map((winner) => winner.year))];
  const visibleWinners = selectedWinnerYear === 'All Years'
    ? winners.images
    : winners.images.filter((winner) => winner.year === selectedWinnerYear);
